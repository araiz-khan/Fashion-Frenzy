...
import { createProduct, deleteProduct, getProductsBySeller } from '@/actions/productActions';
...
export default function SellerDashboardPage() {
  ...
  const fetchSellerProducts = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if ('error' in result) {
        ...
        setSellerProducts(result);
      }
    } catch (error: any) {
      console.error("Error fetching seller products:", error);
      const userMessage = "Failed to load your products. Please try again.";
  ...
  }, [currentUser?.id, fetchSellerProducts, isSeller]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        formData.append('folder', `products/${currentUser.id}`);
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    try {
      ...
      toast({ title: "Deletion Failed", description: "Could not delete product. Please try again.", variant: "destructive" });
    } finally {
      setIsDeleting(false);
  ...
  if (!loadingAuth && currentUser && !isSeller) {
    return (
